...
class PCNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.out = nn.Linear(in_features=128, out_features=PIECE_SELECTOR_NUM_ACTIONS)
        init_weights(self.out, 0)

    def count_output_neurons(self, board_dimensions):
        if not next(self.parameters()).is_cuda:
            x = Variable(torch.rand(1, *board_dimensions))
        else:
    ...
    def forward(self, t):
        ...
        t = t.reshape(-1, self.count_output_neurons(BOARD_DIMENSIONS))
        t = F.relu(self.fc1(t))
        t = self.out(t)
